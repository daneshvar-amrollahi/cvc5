...
//     std::vector<std::string> encodingList; // To collect encodings in the correct order
...
//             else if (n.isConst())
//             {
//                 // Constants are processed immediately
//                 it->second = true;
//                 stack.pop();
//             }
//             else
...
//                 if (child.isConst())
//                 {
//                     // Include 'C' followed by the constant value
//                     std::string value = child.toString();
//                     nodeEncoding += "C" + value + ",";
//                 }
//                 else if (child.isVar())
//                 {
//                     // Include variable ID
...
//                 else
//                 {
//                     // Subtree: Include 'S' followed by its ID
//                     uint32_t childId = subtreeIdMap[child];
//                     nodeEncoding += "S" + std::to_string(childId) + ",";
//                 }
//             }
    ...
    PreprocessingPassContext* d_preprocContext)
{
    ...
    while (!stack.empty())
    {
        ...
        // Check if the node has already been normalized
        auto normIt = normalized.find(current);
        if (normIt != normalized.end())
        {
            // Node has already been processed, skip processing
            continue;
        }
        ...
        {
            // Before processing children
            // If the node has already been normalized, skip processing
